...
- [x] **[T13-13] Idioma y tema: casillas con exclusión a mano donde hay un control para eso** — **hecho (2026-09-19)** · Baja
  ...
  - **Esfuerzo:** bajo
  - **Depende de:** ninguna
  - **Hecho.** Los ocho ítems pasan a `RadioMenuFlyoutItem` con su `GroupName` (`LangGroup`, `ThemeGroup`),
    y la exclusión a mano se va: `SyncThemeMenu` y el bloque de idioma marcan **solo el activo**, de cinco y
    tres asignaciones a una.
  - **Dos cosas medidas con la app en marcha, las dos contra lo que decía esta ficha:**
    - **El árbol de automatización no cambia.** El peer de `RadioMenuFlyoutItem` en la 1.8 sigue siendo
      `MenuItem` con patrón *Toggle*, igual que el del `ToggleMenuFlyoutItem` — no `RadioButton` con
      `SelectionItem`, que es lo que se esperaba. `ClickMenuPath` no necesita ningún ajuste, y un lector de
      pantalla anuncia lo mismo que antes. Lo que sí cambia es lo que se ve: el **punto de opción** en vez
      de la marca de verificación (capturado con el submenú *Tema* abierto).
    - **El `GroupName` no hace falta** para que funcione: quitándolo de los ocho la prueba sigue pasando,
      porque WinUI acota el grupo por defecto a cada flyout — no se mezclan idioma y tema. Se deja por ser
      explícito.
  - **Verificado por reversión:** prueba de UI nueva (`TheChoiceMenus_LeaveExactlyOneMarkedInEachGroup`):
...
- [x] **[T13-18] *Reinicializar* rechaza el plan sin decir por qué, y el sistema sugerido lleva a ese rechazo** — **hecho (2026-09-21)** · Media
  ...
  - **Esfuerzo:** bajo
  - **Depende de:** ninguna
  - **Hecho.** `Core/PlanRejection` reparte cada `PlanProblem` en su clave de texto y sus argumentos —el
    límite y el tamaño del **volumen culpable**, sacado de `EffectiveSizes`, que es el mismo cálculo que
    validó el plan—. Siete mensajes nuevos × 5 idiomas. Los cinco motivos que el formulario no puede
    producir se quedan con el genérico a propósito: si aparecieran serían un fallo de la aplicación, y
    escribirle al usuario una instrucción que no puede seguir sería mentirle. Cuáles son está en
    `PlanRejection.FromTheForm`, que es lo que recorre la prueba.
  - **22 pruebas unitarias** que barren el enum entero: cada motivo tiene texto en los cinco idiomas, sin
    `{0}` sin sustituir, y los del formulario no pueden quedarse en el genérico. Añadir un `PlanProblem`
    obliga a decidir qué se le dice a quien lo provoque. **Verificado por reversión:** al quitar el reparto
    de `Fat32VolumeTooLarge` caen tres.
  - **Visto en la app real (2026-09-21).** La USB de hoy mide 29,3 GB, así que FAT32 entera **sí** es
    válida y el caso original no se reproduce con ella. Se reprodujo el **mismo defecto por la otra vía**:
    `D:` (1020 MB) hace que el selector ofrezca **FAT** —decide por el volumen—, y *Reinicializar* valida
    el disco entero. El diálogo dice: «FAT no admite volúmenes de más de 2 GB, y el que se crearía mide
    29,3 GB. Elige FAT32, exFAT o NTFS. No se ha modificado nada.» Límite, tamaño y salida.
  - **La toma `reinit` de la galería ya no depende de la sugerencia:** elige NTFS, que vale para cualquier
    tamaño de disco. Antes fotografiaba el rechazo en vez de la confirmación.

- [x] **[T13-19] Un segundo `StartButton_Click` con un diálogo abierto acaba en `CRASH`** — **reproducido y hecho (2026-09-22)** · Baja
  - **Área:** Robustez
  ...
  - **Esfuerzo:** bajo
  - **Depende de:** ninguna
  - **Reproducido (2026-09-22).** Invocando `StartButton` dos veces seguidas por UIA, el historial pasa de
    uno a **dos** `CRASH`. La prueba mira el historial y no «la ventana sigue viva», porque viva ya estaba:
    la red global de `App` atrapa la excepción y el único rastro es esa línea.
  - **Hecho.** Los **13** `ShowAsync` de la ventana pasan por `ShowOneAsync`, que no abre un segundo
    diálogo mientras haya uno abierto y deja constancia (`DIALOG SKIPPED`, categoría *App*). Devuelve
    `None`, que es lo que ya devuelve un diálogo cerrado sin elegir: quien confirmaba con `== Primary`
    entiende el rechazo sin cambiar una línea.
  - **Se descarta en vez de encolar:** el segundo diálogo responde a una acción que el usuario puede
    repetir al cerrar el primero, y encolarlo le haría aparecer un diálogo «solo» un rato después. Que
    quede en el historial es lo que lo distingue de tragarse el error.
  - **Un tropiezo que enseñó algo del control:** la primera versión abría la compuerta al volver de
    `ShowAsync`, y entre el cierre del diálogo y esa vuelta hay un salto por la cola del despachador.
    Cerrar un diálogo y pulsar Ctrl+H acto seguido caía en ese hueco: **tres pruebas de la suite fallaron**
    y el historial lo dijo con todas las letras («'Historial de operaciones' … ya había otro abierto:
    'Licencia'», en el mismo segundo). Ahora la compuerta se abre en el evento `Closing`. WinUI, por su
    parte, sí admite abrir el siguiente una vez empezado el cierre; lo que no admite es que haya dos a la
    vez.
  - **Verificado por reversión**, y con dos pruebas: la de UI que cuenta los `CRASH` del historial, y una
