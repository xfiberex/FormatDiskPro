...
public static class L
{
    ...
    internal static readonly Dictionary<string, string[]> Map = new()
    {
        ...
        ["bench.confirmBody"]    = ["Se escribirá y releerá un archivo temporal de unos 256 MB en {0}: para medir la velocidad.\n\nLa operación no es destructiva. ¿Continuar?", "A temporary file of about 256 MB will be written to and read from {0}: to measure speed.\n\nThe operation is non-destructive. Continue?", "Será gravado e relido um arquivo temporário de cerca de 256 MB em {0}: para medir a velocidade.\n\nA operação não é destrutiva. Continuar?", "Un fichier temporaire d'environ 256 Mo sera écrit puis relu sur {0}: pour mesurer la vitesse.\n\nL'opération n'est pas destructive. Continuer ?", "Verrà scritto e riletto un file temporaneo di circa 256 MB su {0}: per misurare la velocità.\n\nL'operazione non è distruttiva. Continuare?"],
        ["bench.writing"]        = ["Benchmark de {0}: — escribiendo…", "Benchmark of {0}: — writing…", "Benchmark de {0}: — gravando…", "Benchmark de {0}: — écriture…", "Benchmark di {0}: — scrittura…"],
        ["bench.reading"]        = ["Benchmark de {0}: — leyendo…", "Benchmark of {0}: — reading…", "Benchmark de {0}: — lendo…", "Benchmark de {0}: — lecture…", "Benchmark di {0}: — lettura…"],
        ...
        ["bench.noSpace"]        = ["No hay espacio libre suficiente en {0}: para el benchmark (se necesitan ~320 MB).", "Not enough free space on {0}: for the benchmark (~320 MB needed).", "Não há espaço livre suficiente em {0}: para o benchmark (são necessários ~320 MB).", "Espace libre insuffisant sur {0}: pour le benchmark (~320 Mo nécessaires).", "Spazio libero insufficiente su {0}: per il benchmark (servono ~320 MB)."],
        ["bench.note"]           = ["Cifras orientativas: la caché del sistema puede inflar la lectura.", "Approximate figures: the system cache may inflate read speed.", "Valores aproximados: o cache do sistema pode inflar a leitura.", "Chiffres indicatifs : le cache système peut gonfler la vitesse de lecture.", "Valori indicativi: la cache di sistema può gonfiare la lettura."],
