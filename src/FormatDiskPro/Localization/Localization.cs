...
public static class L
{
    ...
    internal static readonly Dictionary<string, string[]> Map = new()
    {
        ...
        ["menu.checkUpdatesOnStartup"] = ["Buscar actualizaciones al iniciar", "Check for updates on startup", "Procurar atualizações ao iniciar", "Rechercher les mises à jour au démarrage", "Cerca aggiornamenti all'avvio"],
        ...
        // Por qué el rechazo de Reinicializar tiene siete textos y no uno (`T13-18`): el genérico de
        // arriba decía que el plan no vale, pero no qué cambiar — y el caso corriente (FAT32 sugerida
        // por el tamaño del VOLUMEN, rechazada por el del DISCO) tiene salida a un clic. Cada uno la
        // nombra. El reparto vive en `PlanRejection`, que es donde se prueba.
        ["reinit.plan.fat32TooLarge"] = ["FAT32 no admite volúmenes de más de {0}, y el que se crearía mide {1}. Elige exFAT o NTFS, o marca «Crear partición FAT32 pequeña» para que la FAT32 ocupe solo una parte del disco. No se ha modificado nada.", "FAT32 doesn't support volumes larger than {0}, and the one that would be created is {1}. Choose exFAT or NTFS, or check “Create small FAT32 partition” so the FAT32 takes up only part of the disk. Nothing was changed.", "O FAT32 não suporta volumes maiores que {0}, e o que seria criado tem {1}. Escolha exFAT ou NTFS, ou marque «Criar partição FAT32 pequena» para que a FAT32 ocupe apenas parte do disco. Nada foi alterado.", "FAT32 ne prend pas en charge les volumes de plus de {0}, et celui qui serait créé fait {1}. Choisissez exFAT ou NTFS, ou cochez « Créer une petite partition FAT32 » pour que la FAT32 n'occupe qu'une partie du disque. Rien n'a été modifié.", "FAT32 non supporta volumi più grandi di {0} e quello che verrebbe creato è di {1}. Scegli exFAT o NTFS, oppure seleziona «Crea partizione FAT32 piccola» perché la FAT32 occupi solo una parte del disco. Non è stato modificato nulla."],
        ["reinit.plan.fatTooLarge"] = ["FAT no admite volúmenes de más de {0}, y el que se crearía mide {1}. Elige FAT32, exFAT o NTFS. No se ha modificado nada.", "FAT doesn't support volumes larger than {0}, and the one that would be created is {1}. Choose FAT32, exFAT or NTFS. Nothing was changed.", "O FAT não suporta volumes maiores que {0}, e o que seria criado tem {1}. Escolha FAT32, exFAT ou NTFS. Nada foi alterado.", "FAT ne prend pas en charge les volumes de plus de {0}, et celui qui serait créé fait {1}. Choisissez FAT32, exFAT ou NTFS. Rien n'a été modifié.", "FAT non supporta volumi più grandi di {0} e quello che verrebbe creato è di {1}. Scegli FAT32, exFAT o NTFS. Non è stato modificato nulla."],
        ["reinit.plan.doesNotFit"] = ["Lo que se ha pedido no cabe en este disco ({0}). Reduce el tamaño de la partición FAT32 pequeña. No se ha modificado nada.", "What was requested doesn't fit on this disk ({0}). Reduce the size of the small FAT32 partition. Nothing was changed.", "O que foi solicitado não cabe neste disco ({0}). Reduza o tamanho da partição FAT32 pequena. Nada foi alterado.", "Ce qui a été demandé ne tient pas sur ce disque ({0}). Réduisez la taille de la petite partition FAT32. Rien n'a été modifié.", "Quello che hai chiesto non entra in questo disco ({0}). Riduci la dimensione della partizione FAT32 piccola. Non è stato modificato nulla."],
        ["reinit.plan.tooSmall"] = ["Una de las particiones quedaría por debajo de {0}, que es el mínimo utilizable. Reduce el tamaño de la partición FAT32 pequeña para dejar sitio al resto, o desmarca la segunda partición. No se ha modificado nada.", "One of the partitions would end up below {0}, the minimum usable size. Reduce the size of the small FAT32 partition to leave room for the rest, or uncheck the second partition. Nothing was changed.", "Uma das partições ficaria abaixo de {0}, que é o mínimo utilizável. Reduza o tamanho da partição FAT32 pequena para deixar espaço para o restante, ou desmarque a segunda partição. Nada foi alterado.", "L'une des partitions passerait sous {0}, la taille minimale utilisable. Réduisez la taille de la petite partition FAT32 pour laisser de la place au reste, ou décochez la seconde partition. Rien n'a été modifié.", "Una delle partizioni resterebbe sotto {0}, il minimo utilizzabile. Riduci la dimensione della partizione FAT32 piccola per lasciare spazio al resto, oppure deseleziona la seconda partizione. Non è stato modificato nulla."],
        ["reinit.plan.invalidLabel"] = ["La etiqueta del volumen no es válida para el sistema de archivos elegido: repásala (FAT32 y FAT admiten 11 caracteres; NTFS, exFAT y ReFS, 32). No se ha modificado nada.", "The volume label isn't valid for the chosen file system: check it (FAT32 and FAT allow 11 characters; NTFS, exFAT and ReFS, 32). Nothing was changed.", "O rótulo do volume não é válido para o sistema de arquivos escolhido: verifique-o (FAT32 e FAT aceitam 11 caracteres; NTFS, exFAT e ReFS, 32). Nada foi alterado.", "L'étiquette de volume n'est pas valide pour le système de fichiers choisi : vérifiez-la (FAT32 et FAT acceptent 11 caractères ; NTFS, exFAT et ReFS, 32). Rien n'a été modifié.", "L'etichetta del volume non è valida per il file system scelto: controllala (FAT32 e FAT ammettono 11 caratteri; NTFS, exFAT e ReFS, 32). Non è stato modificato nulla."],
        ["reinit.plan.mbrTooBig"] = ["Este disco mide {1} y la tabla de particiones MBR no puede direccionar más de {0}. No se ha modificado nada.", "This disk is {1} and the MBR partition table can't address more than {0}. Nothing was changed.", "Este disco tem {1} e a tabela de partições MBR não consegue endereçar mais de {0}. Nada foi alterado.", "Ce disque fait {1} et la table de partitions MBR ne peut pas adresser plus de {0}. Rien n'a été modifié.", "Questo disco è da {1} e la tabella delle partizioni MBR non può indirizzare più di {0}. Non è stato modificato nulla."],
        ["reinit.plan.unknownSize"] = ["No se ha podido leer el tamaño de este disco, así que no se puede comprobar que lo pedido quepa. Desmarca la partición FAT32 pequeña y la segunda partición para crear una sola partición con todo el disco. No se ha modificado nada.", "This disk's size couldn't be read, so there's no way to check that what was requested fits. Uncheck the small FAT32 partition and the second partition to create a single partition spanning the whole disk. Nothing was changed.", "Não foi possível ler o tamanho deste disco, então não dá para verificar se o que foi pedido cabe. Desmarque a partição FAT32 pequena e a segunda partição para criar uma única partição com o disco inteiro. Nada foi alterado.", "La taille de ce disque n'a pas pu être lue, impossible donc de vérifier que ce qui est demandé tient. Décochez la petite partition FAT32 et la seconde partition pour créer une seule partition couvrant tout le disque. Rien n'a été modifié.", "Non è stato possibile leggere la dimensione di questo disco, quindi non si può verificare che quanto richiesto ci stia. Deseleziona la partizione FAT32 piccola e la seconda partizione per creare un'unica partizione con tutto il disco. Non è stato modificato nulla."],
        ...
        ["about.privacy"]    = ["No recopila datos personales ni telemetría. La única conexión a Internet es a GitHub Releases (HTTPS), para comprobar y descargar actualizaciones. Se hace al iniciar, y puedes desactivarla en Configuración → «Buscar actualizaciones al iniciar»; entonces solo se conecta si la pides desde Ayuda.", "It collects no personal data or telemetry. The only Internet connection is to GitHub Releases (HTTPS), to check for and download updates. It runs on startup, and you can turn it off in Configuration → “Check for updates on startup”; after that it only connects when you ask it to from the Help menu.", "Não coleta dados pessoais nem telemetria. A única conexão à Internet é ao GitHub Releases (HTTPS), para verificar e baixar atualizações. É feita ao iniciar e pode ser desativada em Configuração → «Procurar atualizações ao iniciar»; depois disso só se conecta quando você pedir no menu Ajuda.", "Aucune donnée personnelle ni télémétrie n'est collectée. La seule connexion Internet est vers GitHub Releases (HTTPS), pour vérifier et télécharger les mises à jour. Elle a lieu au démarrage et peut être désactivée dans Configuration → « Rechercher les mises à jour au démarrage » ; ensuite, la connexion n'a lieu que si vous la demandez depuis le menu Aide.", "Non raccoglie dati personali né telemetria. L'unica connessione a Internet è a GitHub Releases (HTTPS), per verificare e scaricare aggiornamenti. Avviene all'avvio e puoi disattivarla in Configurazione → «Cerca aggiornamenti all'avvio»; dopodiché si connette solo se lo chiedi dal menu Aiuto."],
