...
/// <summary>
/// Ayudas para interactuar con los <c>ContentDialog</c> de WinUI. Confirmado contra hardware real:
/// esta app NO abre ningún HWND top-level nuevo para el diálogo (solo existe la ventana
/// "FormatDiskPro" a nivel de Desktop) — el ContentDialog vive como descendiente de MainWindow, tal
/// como cabía esperar. El árbol de MainWindow contiene MÁS DE UN elemento ControlType.Window a la
/// vez (WinUI deja un proxy de Popup vacío, "Ventana emergente", además del diálogo real); por eso
/// <c>FindFirstDescendant(ByControlType.Window)</c> puede atrapar el proxy vacío en vez del diálogo.
/// Se buscan TODOS los candidatos y se toma el que de verdad tiene contenido. Los botones nativos
/// (Primary/Secondary/Close) heredan su AutomationId del x:Name que WinUI les da en su plantilla,
/// igual que los controles nombrados en el XAML de la app.
/// </summary>
public static class DialogHelper
{
    ...
    private static Window? FindDialog(AppFixture fixture)
    {
        ...
        // Todo diálogo real de esta app (ShowInfoAsync/ShowConfirmAsync/ShowDialogAsync/ConfirmDialog y
        // los ContentDialog XAML propios) fija siempre CloseButtonText y/o Primary/SecondaryButtonText,
        ...
        // tienen ninguno de esos botones — antes se distinguían por "el que tenga más hijos", pero ese
        // heurístico confundía un residuo transitorio con el diálogo real (falsos positivos intermitentes
        // en WaitForNoDialog, que entonces esperaba a que "cerrara" algo que ya no era el diálogo).
        return candidates
            .Select(w => w.AsWindow())
    ...
    /// (p. ej. un MenuFlyout que no ha terminado de cerrarse tras invocar un ítem por patrón UIA en
    /// vez de un clic real) — el heurístico de "más hijos" de <see cref="FindDialog"/> puede entonces
    /// escoger el flyout obsoleto en vez del ContentDialog recién abierto.
    /// </summary>
    ...
    /// <summary>
    /// Reintenta buscar un descendiente por AutomationId dentro de un elemento (normalmente un
    /// diálogo recién detectado): su contenido puede tardar un poco más en poblarse en el árbol de
    /// automatización que el propio elemento ventana que <see cref="WaitForDialog"/> ya detectó.
    /// </summary>
    ...
    /// manejador <c>async void</c> sin captura — puede tirar abajo el proceso entero. Sin esto, un
    /// assert fallido a media prueba deja el diálogo abierto y revienta TODA la suite restante.
    /// </summary>
    ...
    /// <summary>
    /// Vigila y descarta, durante una ventana de tiempo, cualquier diálogo que la propia app abra
    /// por su cuenta (Novedades / Actualización disponible en <c>OnFirstActivated</c>).
    /// </summary>
    ...
    /// <summary>
    /// Lee el texto visible de un TextBlock (ControlType.Text): el patrón Text de UIA (pensado
    /// justo para exponer contenido de solo lectura, potencialmente largo) es más fiable que la
    /// propiedad Name — que para bloques de texto extensos (p. ej. la licencia GPL completa) puede
    /// venir vacía aunque el elemento se localice bien por AutomationId.
    /// </summary>
